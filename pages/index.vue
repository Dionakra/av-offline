  ...
  <div id="app" class="container">
    ...
    <div class="columns is-gapless">
      ...
      <div class="column is-2 is-12-mobile">
        <div class="select is-fullwidth">
          <select v-model="sport" aria-label="Sport">
            ...
            <option v-for="curSport in sports" :value="curSport" :key="curSport">{{curSport}}</option>
          </select>
        </div>
      </div>
      ...
      <div class="column is-2 is-12-mobile">
        <div class="select is-fullwidth">
          <select v-model="competition" aria-label="Competition">
            ...
            <option
              v-for="curComp in competitions[sport]"
              :value="curComp"
              :key="curComp"
            >{{curComp}}</option>
          </select>
      ...
      <div class="column is-7 is-12-mobile">
        <input type="text" placeholder="Search" v-model="text" class="input" aria-label="Search" />
      </div>
    </div>
...
export default {
  async asyncData({ req, params, store, $axios }) {
  ...
  methods: {
    eventsToShow() {
      return this.events.filter(event => {
        ...
        return (
          res && event.event.toLowerCase().includes(this.text.toLowerCase())
        );
      });
    },
    initCap(str) {
